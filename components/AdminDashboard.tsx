...
import { Home, Clock, ArrowUpRight, TrendingUp, Calendar, Heart, ShieldCheck, Zap } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
    ...
    // Helper to calculate real-time percentage trends
    ...
    useEffect(() => {
        const fetchStats = async () => {
            ...
            try {
                // 1. Fetch Visits with historical comparisons
                ...
                const endTime = Date.now(); // End latency timer
                ...
                // Calculate Load (Assuming 250 Gemini requests per day free limit)
                const loadPercentage = Math.min((generatedToday / 250) * 100, 100).toFixed(0);
                ...
                setStats({
                    ...
                    health: {
                        ...
                        load: `${loadPercentage}%`,
                        aiStatus: (today.count ?? 0) > 1 ? 'Operational' : 'Idle'
                    }
                });
            } catch (error) {
            ...
            <div className="max-w-7xl mx-auto px-6 py-10 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    <StatCard
                        ...
                        trend={calculateTrend(stats.visits.today, stats.visits.yesterday)}
                        sub="Since midnight"
                        color="blue"
                    />
                    <StatCard
                        ...
                        trend={calculateTrend(stats.totalGenerated, stats.totalGeneratedYesterday)}
                        sub="Total all-time"
                        color="amber"
                    />
                    <StatCard
                        ...
                        trend={calculateTrend(stats.visits.week, stats.visits.lastWeek)}
                        sub="Past 7 days"
                        color="purple"
                    />
                    <StatCard
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-6">
                        <div className="bg-gradient-to-br from-rose-500/20 to-crimson-600/20 backdrop-blur-2xl border border-white/10 rounded-[3rem] p-8 shadow-2xl">
                            ...
                            <div className="space-y-6">
                                <StatusItem label="Edge Function (Luvv)" status="Active" latency={stats.health.latency} />
                                <StatusItem label="Database Real-time" status="Connected" latency="< 30ms" />
                                <StatusItem label="AI Models (Failover)" status={stats.health.aiStatus} />
                                <StatusItem label="Traffic Load" status={parseInt(stats.health.load) > 80 ? 'High' : 'Normal'} usage={stats.health.load} />
                            </div>
                            <button onClick={() => window.location.reload()} className="w-full mt-10 py-5 bg-white text-black rounded-[2rem] font-bold hover:bg-pink-100 transition-all active:scale-95 shadow-xl">
                                System Refresh
                            </button>
                        </div>
                    </div>
    ...
    return (
        <div className={`p-8 bg-gradient-to-br ${colors[color]} border backdrop-blur-2xl rounded-[3rem] hover:scale-[1.02] transition-all group shadow-xl`}>
            <div className="flex justify-between items-start mb-6">
                <div className="p-4 bg-white/5 rounded-2xl group-hover:scale-110 transition-transform border border-white/10">{icon}</div>
...
const StatusItem: React.FC<{ label: string; status: string; latency?: string; usage?: string }> = ({ label, status, latency, usage }) => (
    <div className="flex justify-between items-center group">
        ...
        <div className="px-4 py-1.5 bg-white/5 border border-white/10 rounded-full"><span className="text-[10px] font-bold text-white uppercase tracking-tighter">{status}</span></div>
    </div>
);
